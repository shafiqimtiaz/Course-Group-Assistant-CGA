<script>
    function validateTACourseSection() {
        ...
        if (student_id == '') {
            ...
            /*for TAs - made sections also optional
             * if no sections are selected, then all sections are selected
             */
...
if (isset($_POST['add'])) {
    ...
    foreach ($check as $row) {
        ...
        if ($check_course_id == $course_id) {
            array_push($errors, "This TA is already assigned to this course.");
        }
        if ($row['section_id'] != NULL) {
            $check_section_id = $row['section_id'];
            if ($check_section_id == $section_id) {
                array_push($errors, "This TA is already assigned to this section.");
            }
        }
    }
...
if (isset($_POST['assign'])) {
    ...
    foreach ($check as $row) {
        $check_section_id = $row['section_id'];
        // Check to see if the TA is already assigned the same section
        if ($check_section_id == $section_id) {
            array_push($errors, "TA is already assigned to this section.");
        }
    }
    ...
    if (count($errors) == 0) {
        ...
        $ta_id = mysqli_fetch_assoc(get_records_where('ta', 'user_id', $user_id))['ta_id'];
        $add_ta = "INSERT INTO ta_of_section (ta_id, section_id) VALUES('$ta_id', '$section_id')";
        if (mysqli_query($conn, $update_ucs) && mysqli_query($conn, $add_ta)) {
            array_push($success, "Assigned Successfully");
        } else {
            array_push($errors, "Could not ASSIGN error: " . mysqli_error($conn));
        }
    }
}
...
<div class="content-body">
    ...
    if (isProfessor()) {
        $query = "SELECT u.*, t.*, c.*, s.section_id, s.section_name FROM users as u
        JOIN ta as t ON t.user_id = u.user_id
        JOIN user_course_section as ucs ON ucs.user_id = u.user_id
        JOIN course as c ON c.course_id = ucs.course_id
        LEFT JOIN section as s ON s.section_id = ucs.section_id
        ...
        <tbody>
            ...
            ?>
                <tr>
                    ...
                    <?php } else { ?>
                        <td><a href="?page=assign-tas&assign_view=true&user_id=<?= $user_id ?>&course_id=<?= $course_id ?>">Assign Section</a></td>
                    <?php } ?>
    ...
    <?php if (isset($_GET['update_view'])) { ?>
        ...
        if (mysqli_num_rows($results) > 0) {
            ...
            $ta_name = $user_id = $course_id = $course_name = $update_section_name = "";
        }
